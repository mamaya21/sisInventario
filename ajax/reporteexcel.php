...
if(1 > 0 ){
		...
		$titulosColumnas = array('# DE FACTURA', 'CLIENTE', 'FECHA', 'SUBTOTAL','IGV','TOTAL');
		...
		while ($fila = odbc_fetch_array($resultado)) {
			$objPHPExcel->setActiveSheetIndex(0) 
        		    ->setCellValue('A'.$i,  $fila['nfactura'])
		            ->setCellValue('B'.$i,  utf8_encode($fila['cliente']))
        		    ->setCellValue('C'.$i,  $fila['fecha'])
            		->setCellValue('D'.$i, $fila['subtotal'])
            		->setCellValue('E'.$i, $fila['igv'])
