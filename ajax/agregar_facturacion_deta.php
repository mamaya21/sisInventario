...
if (isset($_POST['imp'])){$imp=number_format($_POST['imp'],2);}
...
if (!empty($id) and !empty($guia) and !empty($cantidad) and !empty($peso) and !empty($nguia))
{
	...
	$imp= str_replace(',', '',$imp);
	$ssql="insert into detalle_factura values('$n_fac_real',$id,'$guia',$cantidad,$peso,$imp) ";
...
<?php
	...
	$peso=$row['peso'];
	$impor=$row['monto_guia'];
	$n_impor=number_format($impor,2);
	$dest_final="";
	...
	$dest_final = $row_r['cliente'];
		...
		<tr>
			...
			<th class='text-center'><?php echo $n_impor;?></th>
