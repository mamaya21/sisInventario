<?php
	...
	if (empty($_POST['id_remitente'])) {
        ...
        }else if (empty($_POST['importe_fac'])) {
           $errors[] = "No hay importe (Sub-Total) para la Factura";
        }else if (
			!empty($_POST['id_remitente']) &&
			!empty($_POST['fecha_emision']) &&
			!empty($_POST['importe_fac'])
			){ 
		...
		$importe_fac=floatval($_POST["importe_fac"])+0;
		...
		//$sql="insert into t_facturas values ('$n_fac_real',convert(date,'$fec_emision'),$remitente,$importe_fac,$total_igv,$total_fac,'$factura_buscar','N',1) ";
		$sql="insert into t_facturas values ('$n_fac_real',convert(date,'$fec_emision'),$id_remitente_v_agr,$importe_fac,$total_igv,$total_fac,'$factura_buscar','N',1) ";
		...
		if (isset($errors)){
			...
			<meta http-equiv="refresh" content="1;URL=facturas.php" >
			<?php
			}
			if (isset($messages)){
